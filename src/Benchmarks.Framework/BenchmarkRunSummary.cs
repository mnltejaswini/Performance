...
namespace Benchmarks.Framework
{
    ...
    Memory Delta (95th Percentile): {MemoryDeltaPercentile95}";
        ...
        {
            return results.OrderBy(result => result).ElementAt((int)(results.Count() * percentile));
        }

        private static double StandardDeviation(long[] results, long average)
        {
            var summedDifferenceAverage = results
                .Select(result => result - average)
                .Select(result => result * result)
                .Sum();

            return Math.Sqrt(summedDifferenceAverage / results.Length);
        }
    }
}
